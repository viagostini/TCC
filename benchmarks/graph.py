...
for n in N:
    arr = []
    with open(str(root)+ 'naive_4ary_' + str(n) + '_query.txt') as f:
        for line in f:
...
for n in N:
    arr = []
    with open(str(root)+ 'preorder_4ary_' + str(n) + '_query.txt') as f:
        for line in f:
            arr.append(float(line))
...
for n in N:
    arr = []
    with open(str(root)+ 'table_4ary_' + str(n) + '_query.txt') as f:
        for line in f:
            arr.append(float(line))
...
for n in N:
    arr = []
    with open(str(root)+ 'jump_4ary_' + str(n) + '_query.txt') as f:
        for line in f:
            arr.append(float(line))
...
plt.savefig("../modelo-latex/figuras/query_4ary.pdf")
#plt.show()
